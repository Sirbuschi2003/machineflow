...
import { Search, CheckCircle, AlertCircle, ArrowLeft, Plus, Minus } from 'lucide-react';
...
export default function NewRequest() {
  ...
  useEffect(() => {
    Promise.all([api.machineModels.getAll(), api.accessories.getAll()]).then(([m, a]) => {
      setModels(m);
      setAccessories(a);
      setAccessorySelections(a.map((acc) => ({ accessoryId: acc.id, name: acc.name, quantity: 1, selected: false })));
    });
  }, []);
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      if (primary) {
        ...
        setEditedSite({ ...primary });
      }
    } catch (e: unknown) {
      setLookupError(e instanceof Error ? e.message : 'Kunde nicht gefunden.');
    } finally {
  ...
  const handleSubmit = async (asDraft: boolean) => {
    ...
    setSubmitError('');
    try {
      const siteId = editingAddress && editedSite.id
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Schritt {step === 'customer' ? 1 : step === 'machine' ? 2 : 3} von 3</p>
        </div>
      </div>
      ...
      {step === 'machine' && (
        <div className="space-y-4">
            ...
            <div className="card-body space-y-2">
              {accessorySelections.map((acc, idx) => (
                ...
                >
                  ...
                  {accessories.find((a) => a.id === acc.accessoryId)?.hasSerialNumber && (
          ...
          <div className="flex justify-between">
            <button className="btn-secondary" onClick={() => setStep('customer')}>
              ← Zurück
            </button>
            <button
              className="btn-primary"
      ...
      {step === 'confirm' && (
        <div className="space-y-4">
          ...
          <div className="flex justify-between">
            <button className="btn-secondary" onClick={() => setStep('machine')}>
              ← Zurück
            </button>
            <div className="flex gap-2">
              <button
